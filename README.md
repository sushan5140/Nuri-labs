...
The live page is `components/atlas.tsx` with `app/atlas.css`; the bottom editorial story uses `components/storybook.tsx` and `app/storybook.css`. Earlier `flagship.tsx` and `hub.tsx` designs remain in Git history/source as visual references and are not rendered by the current page.
...
- Each chapter follows KMate landing-page storytelling: a concrete difficulty, the human moment behind it, the reason the product exists, and a direct **Step into** CTA that opens the existing One Universe preview. The sticky chapter index makes these stories navigable without creating a second product directory.

...
export default function NuriAtlas(){
 const [storyVersion,setStoryVersion]=useState<'A'|'B'>('B'),[active,setActive]=useState<WorldId>('hallium'),[portal,setPortal]=useState<WorldId|null>(null),[view,setView]=useState<View>('home'),[saved,setSaved]=useState<string[]>([]),[menu,setMenu]=useState(false),[filter,setFilter]=useState('All'),[query,setQuery]=useState('');const searchRef=useRef<HTMLInputElement>(null);
 useEffect(()=>{try{if(localStorage.getItem('nuri.story-version')==='A')setStoryVersion('A')}catch{}try{const v=JSON.parse(localStorage.getItem('nuri.saved-products')||'[]');if(Array.isArray(v))setSaved(v.filter((x:unknown):x is string=>typeof x==='string'))}catch{}const sync=()=>setView(location.hash==='#hub'?'hub':'home');sync();addEventListener('popstate',sync);addEventListener('hashchange',sync);return()=>{removeEventListener('popstate',sync);removeEventListener('hashchange',sync)}},[]);
 ...
 const selectStory=(version:'A'|'B')=>{setStoryVersion(version);try{localStorage.setItem('nuri.story-version',version)}catch{}};
 ...
 <div className="na-ticker"><span>LEARN SOMETHING NEW ✳ FIND YOUR PEOPLE ✳ GET LOST IN A STORY ✳ BUILD YOUR NEXT CHAPTER ✳ LEARN SOMETHING NEW ✳ FIND YOUR PEOPLE ✳ GET LOST IN A STORY ✳ BUILD YOUR NEXT CHAPTER ✳ </span></div><Trail onPick={openWorld}/><div id="story-switch" className="na-story-switch"><div className="na-wrap na-story-switch-inner"><div><span>THE STORY BEHIND NURI</span><p>Two different ways to tell it. Explore both before choosing our final direction.</p></div><div className="na-story-switch-buttons" role="group" aria-label="Choose story design"><button type="button" aria-pressed={storyVersion==='A'} className={storyVersion==='A'?'selected':''} onClick={()=>selectStory('A')}>Version A <small>Four chapters</small></button><button type="button" aria-pressed={storyVersion==='B'} className={storyVersion==='B'?'selected':''} onClick={()=>selectStory('B')}>Version B <small>One unfolding story</small></button></div></div></div>{storyVersion==='A'?<Storybook onOpen={openWorld} onExplore={()=>scroll('universe')}/>:<StoryFlow onOpen={openWorld} onExplore={()=>scroll('universe')}/>}</main>
